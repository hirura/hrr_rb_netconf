...
RSpec.describe HrrRbNetconf::Server::Error::UnknownAttribute do
  let(:tag){ 'unknown-attribute' }
  let(:type){ ['rpc', 'protocol', 'application'] }
  let(:severity){ ['error'] }
  let(:info){ ['bad-attribute', 'bad-element'] }
